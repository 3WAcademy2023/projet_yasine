...
const searchArticle = asyncHandler(async (req, res) => {
  //const articles = await Articles.find({title: req.body.search,}).populate("category");
  

  const keywordSearch= req.body.search
  const articles = await Articles.find({ title: { $regex: keywordSearch , $options: 'i'}}).populate(
    "category"
  );
